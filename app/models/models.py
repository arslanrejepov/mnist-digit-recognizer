...
class Follow(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    follower_id = Column(Integer, ForeignKey("users.id"), nullable=False)  # the one who follows
    followed_id = Column(Integer, ForeignKey("users.id"), nullable=False)  # the one being followed
    created_at = Column(DateTime, default=datetime.utcnow)
