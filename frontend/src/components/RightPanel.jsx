...
// Deterministic color — same username always same color
...
export default function RightPanel({ activePost, showToast }) {
  ...
  // ── Load all real users from DB (public endpoint — no auth needed) ──
  useEffect(() => {
    setLoadingUsers(true)
    getAllUsers()
      .then(async (list) => {
        // Filter out yourself
        const others = list.filter((u) => u.username !== me)
        setUsers(others)

        // Load follower counts in parallel
        const counts = {}
        await Promise.allSettled(
          others.map(async (u) => {
            try {
              const followers = await getFollowers(u.username)
              counts[u.username] = Array.isArray(followers) ? followers.length : 0
            } catch {
              counts[u.username] = 0
            }
          })
  ...
  return (
    <aside className={styles.panel}>
      ...
      <div className={styles.stories}>
        {/* Your own story bubble */}
        <div className={styles.storyItem} onClick={() => showToast('📸 Your story coming soon!')}>
        ...
        {users.slice(0, 8).map((u) => (
          <div key={u.id} className={styles.storyItem} onClick={() => navigate(`/profile/${u.username}`)}>
            <div className={styles.storyRing}>
      ...
      <div className={styles.header}>
        <span className={styles.title}>{tab === 'comments' ? 'Comments' : 'Singers'}</span>
        <div className={styles.tabs}>
          <button className={`${styles.tab} ${tab === 'comments' ? styles.active : ''}`} onClick={() => setTab('comments')}>
            Comments
          </button>
          <button className={`${styles.tab} ${tab === 'suggested' ? styles.active : ''}`} onClick={() => setTab('suggested')}>
            People
          </button>
        </div>
      </div>
            ...
            {comments.map((c, i) => {
              ...
              return (
                <div key={c.id ?? i} className={styles.comment}>
                  <div className={styles.commentAvatar} style={{ background: avatarColor(commentUser) }}>
                    {commentUser[0].toUpperCase()}
                  </div>
                  <div className={styles.commentBody}>
                    <div className={styles.commentMeta}>
                      <span className={styles.commentUser} onClick={() => navigate(`/profile/${commentUser}`)}>
                        @{commentUser}
                      </span>
      ...
      {/* ── People tab — real singers from DB ── */}
      {tab === 'suggested' && (
        <div className={styles.suggestedList}>
          <p className={styles.suggestedTitle}>🎤 Singers on SeS</p>

          {loadingUsers && <p className={styles.empty}>Loading singers…</p>}

          {!loadingUsers && users.length === 0 && (
            <p className={styles.empty}>No other singers yet.</p>
          )}
            ...
            return (
              <div key={u.id} className={styles.suggestedUser}>
                <div
                  ...
                  style={{ background: avatarColor(u.username), cursor: 'pointer' }}
                  onClick={() => navigate(`/profile/${u.username}`)}
                >
                  {u.username[0].toUpperCase()}
                </div>
                <div className={styles.suggestedInfo}>
                  <div className={styles.suggestedName} onClick={() => navigate(`/profile/${u.username}`)}>
                    @{u.username}
                  </div>
                  <div className={styles.suggestedSub}>{formatCount(count)} followers</div>
                </div>
                <button
